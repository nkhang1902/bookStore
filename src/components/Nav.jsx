...
const Navbar = () => {
  ...
  const live_search = (e) =>{
    if(e.target.value == []){
      ...
      setSearch(true)
      var search_name = books.filter(book => book.data.Name.toLowerCase().includes(e.target.value.toLowerCase()))
      var search_auth = books.filter(book => book.data.Author.toLowerCase().includes(e.target.value.toLowerCase()))
          ...
          {searh && (
            Object.keys(data).length === 0 ? (
              ...
              <p className='res-item'>
                <ul>
                  <li className='not_find'>Can't find the book</li>
                </ul>
              </p>
              </div>
            ) : (
                ...
                {data.slice(0,5).map((book)=>(
                    <a  href={`/BookDetails/${book.id}`}>
                        <li className='search_item'>
                          <img src={book.data.ImageURL} alt=""  className='search_img'/>
                          <div className='search_inf'>
                            <h1 className='search_name'>{book.data.Name}</h1>
                            <h1 className='search_auth'>{book.data.Author}</h1>
                          </div>
                          <div className='price'>
                            <p className='search_price'> ${book.data.DiscountPrice}</p>
                          </div>
                        </li>
                      <hr />
                    </a>
